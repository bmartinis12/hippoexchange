...
const ImageSlider = ({ urls }: ImageSliderProps) => {
    ...
    const inactiveStles = 'hidden text-gray-400'
        ...
        <div className="group relative bg-zinc-100 aspect-square overflow-hidden rounded-xl">
            <div className="absolute z-10 inset-0 opacity-0 group-hover:opacity-100 transition">
                <button onClick={(e) => {
                    e.preventDefault();
                    swiper?.slideNext();
                }} aria-label='next image' className={cn(activeStyles, "right-3 transition",
                    {
                        [inactiveStles]: slideConfig.isEnd,
                ...
                <button onClick={(e) => {
                    e.preventDefault();
                    swiper?.slidePrev();
                }} aria-label='previous image' className={cn(activeStyles, "left-3 transition",
                    {
                        [inactiveStles]: slideConfig.isBeginning,
            ...
            <Swiper pagination={{ renderBullet: (_, className) => { return `<span class="rounded-full transition ${className}"></span>` } }} onSwiper={(swiper) => setSwiper(swiper)} spaceBetween={50} slidesPerView={1} modules={[Pagination]} className='h-full w-full'>
                {urls.map((url, i) => (
                    <SwiperSlide key={i} className='-z-10 relative h-full w-full'>
                        <Image src={url} alt='product image' fill sizes='100%' loading='eager' priority className='-z-10 h-full w-full object-cover object-center' />
                    </SwiperSlide>
                ))}
            </Swiper>
        </div>
