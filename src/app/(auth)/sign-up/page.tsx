...
const Page = () => {
    ...
    const onSubmit = ({ email, password }: TAuthCredentialsValidator) => {
        ...
        mutate({ email: lowerCaseEmail, password });
    };
                        ...
                        <form onSubmit={handleSubmit(onSubmit)}>
                            <div className="grid gap-2">
                                <div className="grid gap-1 py-2">
                                    ...
                                    <Input {...register("email")} className={cn({ "focus-visible:ring-red-500": errors.email })} placeholder="Email" />
                                    {errors?.email && (
                                        <p className="text-small text-red-500">{errors.email.message}</p>
                                    )}
                                </div>
                                <div className="grid gap-1 py-2">
                                    ...
                                    {errors?.password && (
                                        <p className="text-small text-red-500">{errors.password.message}</p>
                                    )}
                                </div>
